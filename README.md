...
> CommandDeck 会执行用户配置或导入的命令。只导入和运行你信任的工具包。导入只保存配置，工具需要你自行放到本机目录后再定位。
...
- 命令模板中的 `{{TOOL_DIR}}`、`{{WORKSPACE}}`、`{{DOWNLOAD_URL}}` 在运行时展开。`{{TOOL_DIR}}` 优先使用该工具定位的本地目录，否则回退到工作区下的 `tools/<工具名>`。
- 每条命令可同时保存 macOS/Linux 与 Windows 文本；当前系统会选择对应版本。
- 导入失败会回滚本次写入的工具与分类，避免半导入状态。
