...
- 左键点击工具直接启动；已有运行实例时进入现有会话。
- 工具图标支持跨分类拖拽，禁用原生图片拖拽，避免出现系统复制光标。
- CPU、内存和工具状态展示使用圆角状态框。
- 增加本机 MCP 调试能力：Agent 可读取状态、新建工具、添加命令、运行保存或临时命令，并在界面查看调用记录。
- 正式应用与构建产物统一使用 `CommandDeck` 命名。

> 命令仍只在本机由用户配置或 Agent 明确发起时执行；MCP 调试接口仅监听 `127.0.0.1:19527`。
